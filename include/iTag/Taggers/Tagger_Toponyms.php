...
class Tagger_Toponyms extends Tagger {
    ...
    private function process($footprint, $options) {
        ...
        $query = $prequery . ' SELECT geonameid, name, longitude, latitude, fcode, population, ST_Distance(geom, corrected_centroid) as distance FROM prequery, gazetteer.geoname WHERE st_intersects(geom, corrected_geometry) AND fcode IN ' . $codes . ' ORDER BY distance ASC';
